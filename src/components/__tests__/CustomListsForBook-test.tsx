...
describe("CustomListsForBook", () => {
  ...
  beforeEach(() => {
    ...
    editCustomListsForBook = stub().returns(
  ...
  describe("rendering", () => {
    ...
    it("shows current lists", () => {
      ...
      const removable = wrapper.find(WithRemoveButton);
      expect(removable.length).to.equal(1);
      const link = removable.find("a");
    ...
    it("creates a URL based on list ID", () => {
      const url1 = wrapper.instance().makeURL("list 1");
    ...
    it("shows a menu of available lists", () => {
      const availableLists = wrapper.find("select");
      expect(availableLists.length).to.equal(1);
      expect(
        availableLists.find("option").map((o) => o.prop("value"))
      ).to.deep.equal(["list 1", "list 3"]);
      const label = availableLists.closest("label");
      expect(label.length).to.equal(1);
      expect(label.text()).to.contain("Select an existing list");
      const button = wrapper.find(".add-list-item-container").find(Button);
    ...
    it("does not show the menu if the book is already on all the lists", () => {
      ...
      const message = wrapper.find("span.add-list-item");
    ...
    it("does not show the InputList if no lists exist", async () => {
      ...
      expect(wrapper.find("p").at(0).text()).to.equal(
    ...
    it("displays a link to the list creator", () => {
      const linkDiv = wrapper.find("div").last();
      const link = linkDiv.find(Link);
      expect(link.prop("to").pathname).to.equal(
  ...
  describe("behavior", () => {
    it("fetches lists on mount", () => {
      ...
      expect(fetchCustomListsForBook.args[0][0]).to.equal(
    ...
    it("removes a list", async () => {
      ...
      let removables = wrapper.find(WithRemoveButton);
      expect(removables.length).to.equal(1);
      const removeButton = removables.find(Button);
      await removeButton.simulate("click");

      removables = wrapper.find(WithRemoveButton);
      expect(removables.length).to.equal(0);
      const links = wrapper.find(WithRemoveButton).find("a");
      expect(links.length).to.equal(0);

      expect(editCustomListsForBook.callCount).to.equal(1);
      expect(editCustomListsForBook.args[0][0]).to.equal(
        "admin/works/book url/lists"
      );
      const formData = editCustomListsForBook.args[0][1];
      expect(JSON.parse(formData.get("lists"))).to.deep.equal([]);
