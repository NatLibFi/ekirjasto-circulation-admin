...
export interface LaneEditorProps
  extends React.Props<LaneEditor>,
...
export class LaneEditor extends React.Component<
  LaneEditorProps,
  LaneEditorState
> {
          ...
          <div>
            <div className="save-or-edit">
              <TextWithEditMode
                ...
                aria-label={t("laneEditor.enterLaneName")}
              />
            ...
            <span className="lane-buttons">
              {lane && this.hasCustomLists() && this.props.deleteLane && (
                <Button
                  ...
                  content={
                    <span>
                      <Trans i18nKey="laneEditor.deleteLane">
              ...
              <Button
                className="save-lane"
                callback={this.save}
                content={t("laneEditor.saveLane")}
              />
              {this.hasChanges() && (
                <Button
                  className="cancel-changes inverted"
                  callback={this.reset}
                  content={t("laneEditor.cancelChanges")}
                />
              )}
            </span>
          </div>
          <div className="lane-details">
            ...
            {this.hasCustomLists() && parent && (
              <EditableInput
                ...
                label={t("laneEditor.inheritRestrictions")}
  ...
  renderCustomListsEditor(): JSX.Element {
    const { editOrCreate, lane, t } = this.props;
    ...
    if (editOrCreate === "edit") {
      ...
      if (!this.hasCustomLists()) {
        return <div>{t("laneEditor.autoGeneratedLaneDescription")}</div>;
      }
    }
  ...
  renderInfo(parent?: LaneData): JSX.Element {
    ...
    if (this.props.lane) {
      ...
      if (isVisible(this.props.lane)) {
        visibility = (
          <div>
            <Trans i18nKey="laneEditor.laneIsVisible">
        ...
        visibility = (
          <div>
            <Trans i18nKey="laneEditor.laneParentIsHidden">
        ...
        visibility = (
          <div>
            <Trans i18nKey="laneEditor.laneIsHidden">
      ...
      return (
        <div>
          {parent
            ? t("laneEditor.newSubLane", {
                parent: parent.display_name,
              })
            : t("laneEditor.newTopLevelLane")}
        </div>
      );
    }
  }
        ...
        <Button
          ...
          content={`${
            this.props.lane.visible
              ? t("laneEditor.hideLane")
              : t("laneEditor.showLane")
          }`}
          callback={() =>
...
export default withTranslation()(LaneEditor);
